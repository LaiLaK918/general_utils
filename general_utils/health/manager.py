...
class HealthManager:
    ...
    async def run_checks(self) -> Dict[str, Any]:
        ...
        summary = HealthCheckResult(
            status=overall, timestamp=time.time(), components=results
        )
        self._cache = CachedHealthCheckResult(time=time.time(), data=summary)
        return summary

    def get_cached(self) -> CachedHealthCheckResult:
